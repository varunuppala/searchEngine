...
def combine_json(output):
    ...
    for i,file in enumerate(files):
        ...
        if i == 0:
            with open("output/phrase/final.json","w") as final:
                json.dump(present,final)
        else:
            with open("output/phrase/final.json") as df1:
                dicto = json.load(df1)
            ...
            with open("output/phrase/final.json","w") as df2:
                json.dump(dicto,df2)
...
def to_json(dict):
    ...
    with open("output/phrase/%s.json" %fileno[len(fileno)-1], "w") as outfile:
        json.dump(dict, outfile)


def describefile():
        with open("output/phrase/final.json") as final:
        ...
        for i,j in dict.items():
            if i not in lexicon:
                ...
                lexicon[i]+=len(j)
        with open("output/phrase/lexicon.json","w") as out:
            json.dump(lexicon,out)

        print("\n")
        print("# size of lexicon : ",len(dict))
        print("\n")
        print("size of file in bytes : ",os.path.getsize("output/phrase/final.json")+os.path.getsize("output/phrase/lexicon.json"))
        print("\n")
