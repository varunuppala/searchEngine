...
def getStopWords():
    ...
    """
    stopWords = {}
    with open("P1-packages/stops.txt") as f:
        for line in f:
            val = line.split()
            stopWords[val[0]] = 1
    return stopWords
...
def tokenize(doc, docno, m):
    ...
    for j in range(len(finaltokens)-1):
        if finaltokens[j] and finaltokens[j]!= " " and finaltokens[j] not in stopwords:
            ...
            if docno not in index:
                index[docno] = {}
            else:
                if s not in index[docno]:
                    index[docno][s] = 1
                else:
                    index[docno][s] += 1            
...
def combine_json(output):
    ...
    for i,file in enumerate(files):
        ...
        if i == 0:
            with open("output/single/final.json","w") as final:
                json.dump(present,final,indent = 4)
        else:
            with open("output/single/final.json") as df1:
                dicto = json.load(df1)
            ...
            with open("output/single/final.json","w") as df2:
                json.dump(dicto,df2)
...
def to_json(dict):
    ...
    with open("output/single/%s.json" %fileno[len(fileno)-1], "w") as outfile:
...
def describefile():
        ...
        """
        with open("output/single/final.json") as final:
        ...
        for i,j in dict.items():
            if i not in lexicon:
                ...
                lexicon[i]+=len(j)
        with open("output/single/lexicon.json","w") as out:
            json.dump(lexicon,out, indent = 4)

        """
        print("# size of lexicon : ",len(dict))
        print("\n")
        print("size of file in bytes : ",os.path.getsize("output/single/final.json")+os.path.getsize("output/single/lexicon.json"))
        print("\n")
...
def doclist_json(doclist):
    ...
    json.dump(doclist, f,indent = 4)
    f.close()

def defineDocuments(index):
