...
def identifyNarr(generator):
    """
    1. Identify each narr and store them
    """
    stack = []
    string = ""
    qno = re.compile(r"<num> Number: ([\S]+)")#re for query number
    ...
    for i in generator:
        ...
        nmatch = qno.search(i) #check query number
        if nmatch:
            number = nmatch.group(1) #extracting the number
        if i == "<narr> Narrative:\n":
...
def singleindexQuery(queryDict):
    ...
    for number,query in queryDict.items():
        ...
        queryDict[number] = list(dict.fromkeys(queryDict[number]))
...
def indexType1(qpath,index):

    queries = querying1(qpath) #pass path
